...
const VideoPin = () => {
  ...
  useEffect(() => {
    ...
    getSpecificVideo(videoId).then((data) => {
      setVideoInfo(data);
      recommendedFeed(data.category, videoId).then((feed) => {
        setFeeds(feed);
      });
  ...
  if (!videoId || typeof videoId !== 'string') {
    return <div style={{color: 'red', fontWeight: 'bold'}}>Invalid video ID</div>;
  }

  // Add a handler for play button
  const handleUserPlay = () => setIsPlaying(true);
